...
echo "=== [2/4] 기본 패키지 + Nginx + Certbot 설치 ==="
if [[ "$OS_ID" == "amzn" ]]; then
...
if [[ ! -f "$HOME/nginx/nginx.conf" ]]; then
  ...
  echo "   스크립트 실행 전에 nginx/ 디렉토리가 홈 디렉토리에 준비되어 있는지 확인하세요."
  exit 1
fi
...
echo "  3. 서버 이관/운영 전환 절차에 따라 애플리케이션을 직접 반영"
