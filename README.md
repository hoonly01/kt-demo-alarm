...
## 🧰 개발 환경
이 저장소는 Python 버전과 패키지 관리를 uv 기준으로 맞춥니다.

| 항목 | 기준 |
|---|---|
| 기본 Python 런타임 | `.python-version`의 `3.12` |
| 패키지 호환 범위 | `pyproject.toml`의 `requires-python = ">=3.12"` |
| 의존성 동기화 | `uv sync --locked --dev` |
| 테스트 | `uv run python -m pytest tests/ -v` |

CI는 `uv python install`로 프로젝트의 Python pin을 설치한 뒤 lockfile 기준으로 의존성을 동기화합니다.
