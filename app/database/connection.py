...
def init_db():
    ...
    try:
        cursor.execute("ALTER TABLE events ADD COLUMN attendees INTEGER")
        logger.info("✅ events 테이블 attendees 컬럼 추가 완료")
    except sqlite3.OperationalError as e:
        if "duplicate column name" not in str(e).lower():
            logger.warning(f"events 테이블 attendees 컬럼 갱신 실패 (무시됨): {str(e)}")
